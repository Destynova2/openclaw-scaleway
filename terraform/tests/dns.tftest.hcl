...
run "dns_module_app_cname" {
  command = plan
  assert {
    condition     = module.dns.records["app_cname"].name == "app"
    error_message = "CNAME record should be for app subdomain"
  }
  assert {
    condition     = module.dns.records["app_cname"].type == "CNAME"
    error_message = "App record should be CNAME type"
  }
  assert {
    condition     = module.dns.records["app_cname"].ttl == 300
    error_message = "CNAME TTL should be 300"
  }
}

run "dns_module_webhooks_a" {
  command = plan
  assert {
    condition     = module.dns.records["webhooks_a"].name == "webhooks"
    error_message = "A record should be for webhooks subdomain"
  }
  assert {
    condition     = module.dns.records["webhooks_a"].type == "A"
    error_message = "Webhooks record should be A type"
  }
  assert {
    condition     = module.dns.records["webhooks_a"].ttl == 300

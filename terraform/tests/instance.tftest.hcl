...
run "instance_type_dev1s" {
  command = plan
  assert {
    condition     = scaleway_instance_server.openclaw.type == "DEV1-S"
    error_message = "Instance type should be DEV1-S"
  }
}

run "instance_image_ubuntu_noble" {
  command = plan
  assert {
    condition     = scaleway_instance_server.openclaw.image == "ubuntu_noble"
    error_message = "Instance image should be ubuntu_noble"
  }
}

run "instance_has_private_network" {
  command = plan
  assert {
    condition     = length(scaleway_instance_server.openclaw.private_network) > 0
    error_message = "Instance should have a private_network block with pn_id"
  }
}

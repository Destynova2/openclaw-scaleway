...
run "outputs_defined" {
  command = plan
  assert {
    condition     = output.openclaw_web_ui == "https://app.example.com"
    error_message = "openclaw_web_ui should be https://app.example.com"
  }
  assert {
    condition     = output.webhooks_url == "https://webhooks.example.com"
    error_message = "webhooks_url should be https://webhooks.example.com"
  }
}

...
run "project_name" {
  ...
  assert {
    ...
    error_message = "Project name should be openclaw-production"
  }
}

run "project_has_description" {
  command = plan
  assert {
    condition     = scaleway_account_project.openclaw.description != ""
    error_message = "Project should have a description"
  }
}

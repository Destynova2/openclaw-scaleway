...
run "killswitch_runtime_python" {
  command = plan
  assert {
    condition     = scaleway_function.killswitch[0].runtime == "python310"
    error_message = "Kill switch function runtime should be python310"
  }
}

run "killswitch_handler" {
  command = plan
  assert {
    condition     = scaleway_function.killswitch[0].handler == "handler.handler"
    error_message = "Kill switch handler should be handler.handler"
  }
}

run "killswitch_public" {
  command = plan
  assert {
    condition     = scaleway_function.killswitch[0].privacy == "public"
    error_message = "Kill switch function should be public (webhook target)"
  }
}

run "killswitch_iam_module" {
  command = plan
  assert {
    condition     = module.iam_killswitch[0].application_id != ""
    error_message = "Kill switch IAM module should produce an application_id"
  }
}
